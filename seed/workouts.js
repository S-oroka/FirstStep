...
const createWorkoutPlans = async(workouts) => {
    const filteredWorkoutsUpper = workouts.filter(workout => workout.muscleGroup === "Upper-Body")
    const workoutIdsUpper = filteredWorkoutsUpper.map(workout => workout._id)
    const filteredWorkoutsLower = workouts.filter(workout => workout.muscleGroup === "Lower-Body")
